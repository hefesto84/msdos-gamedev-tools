...
{	double lb,ub,x0,*xval,*tval;
	int nint,nret,errcode;

	clrscr();
	puts("Test program for function adams");
	puts("-------------------------------\n");
	...
	xval=farcalloc(nret+1,sizeof(double));
	tval=farcalloc(nret+1,sizeof(double));
	if (xval==NULL || tval==NULL)
	{	fputs("Not enough core", stderr);
		exit(-1);
	}

	adams(lb, ub, x0, nret, nint, tval, xval, &errcode, bf);
