...
static void usage()
{
   ...
   {
      "",
      "MIDI patch grabbing utility for the Allegro DIGMID driver " ALLEGRO_VERSION_STR,
      "By Shawn Hargreaves, " ALLEGRO_DATE_STR,
      "",
      "Usage: pat2dat [options] filename.dat [samplelocation] [tunes.mid]",
      "",
      "Supported input formats:",
      "\tGUS patch: <location> should point to the default.cfg index file",
      "\tSoundFont: <location> should point to a SoundFont .sf2 file",
      "",
      "Options:",
      "\t'-b x' grab specified instrument bank",
      "\t'-c' compress objects",
      "\t'-d x' grab specified percussion kit",
      "\t'-o' overwrite existing datafile",
      "\t'-v' select verbose mode",
      "\t'-vv' select very verbose mode",
      "\t'-8' reduce sample data to 8 bits (recommended)",
      NULL
   };
...
{
   static char msg[] = "Converted by Allegro pat2dat v" ALLEGRO_VERSION_STR;
   sfSample *sample;
   sfGenList *igen;
   sfGenList *pgen;
   float vol, total_vol;
...
static void add_soundfont_patches()
{
   ...
   fprintf(f, "# Produced by pat2dat v" ALLEGRO_VERSION_STR "\n");
   fprintf(f, "# Date: %s\n\n", tm);
...
int main(int argc, char *argv[])
{
   ...
   if (!opt_overwrite) {
	 ...
	 fprintf(stderr, "%s already exists: use -o to overwrite\n", opt_datafile);
	 return 1;
      }
   }
   ...
   if (!err) {
	 ...
	 if (!datedit_save_datafile(datafile, opt_datafile, 1, opt_compression, (opt_veryverbose || (opt_verbose && opt_compression)), TRUE, FALSE, NULL))
	    err = 1;
      }
