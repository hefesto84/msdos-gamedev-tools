...
static char none_desc[] = "The sound of silence";

int  _dummy_detect(int input) { return TRUE; }
int  _dummy_init(int input, int voices) { digi_none.desc = get_config_text(none_desc); midi_none.desc = get_config_text(none_desc); return 0; }
...
MIDI_DRIVER *midi_input_driver = &midi_none;

void (*digi_recorder)() = NULL;
...
static void read_sound_config()
{
   ...
   _sound_hq = get_config_int("sound", "quality", _sound_hq);
   ...
   _mpu_irq = get_config_hex("sound", "mpu_irq", -1);
...
int detect_digi_driver(int driver_id)
{
   ...
   for (i=0; _digi_driver_list[i].driver_id; i++) {
	 ...
	 if (_digi_driver_list[i].driver->detect(FALSE))
...
int detect_midi_driver(int driver_id)
{
   ...
   for (i=0; _midi_driver_list[i].driver_id; i++) {
	 ...
	 midi_card = driver_id;
	 if (_midi_driver_list[i].driver->detect(FALSE))
...
/* fix_digi_id:
 *  Backward-compatibility wrapper for using old-style config numbers.
 */
static int fix_digi_id(int id)
{
   switch (id) {
      case 1: return DIGI_SB;
      case 2: return DIGI_SB10;
      case 3: return DIGI_SB15;
      case 4: return DIGI_SB20;
      case 5: return DIGI_SBPRO;
      case 6: return DIGI_SB16;
      case 8: return DIGI_AUDIODRIVE;
   }
 ...
 *  Backward-compatibility wrapper for using old-style config numbers.
 */
static int fix_midi_id(int id)
{
   switch (id) {
      case 1: return MIDI_ADLIB;
      case 2: return MIDI_OPL2;
      case 3: return MIDI_2XOPL2;
      case 4: return MIDI_OPL3;
      case 5: return MIDI_SB_OUT;
      case 6: return MIDI_MPU;
      case 8: return MIDI_DIGMID;
      case 9: return MIDI_AWE32;
   }
...
int install_sound(int digi, int midi, char *cfg_path)
{
   ...
   if (digi_card == DIGI_AUTODETECT)
      digi_card = fix_digi_id(get_config_id("sound", "digi_card", DIGI_AUTODETECT));

   if (midi_card == MIDI_AUTODETECT)
      midi_card = fix_midi_id(get_config_id("sound", "midi_card", MIDI_AUTODETECT));
   ...
   if (!digi_driver) {
	 ...
	 if ((_digi_driver_list[c].autodetect) &&
	     (_digi_driver_list[c].driver->detect(FALSE))) {
	    digi_driver = _digi_driver_list[c].driver;
	    break;
	 }
      }
   ...
   if (!midi_driver) {
	 ...
	 if ((_midi_driver_list[c].autodetect) &&
	     (_midi_driver_list[c].driver->detect(FALSE))) {
	    midi_driver = _midi_driver_list[c].driver;
	    break;
	 }
      }
   ...
   if ((digi_voices > DIGI_VOICES) || (midi_voices > MIDI_VOICES)) {
      sprintf(allegro_error, get_config_text("Insufficient %s voices available"), 
...
int install_sound_input(int digi, int midi)
{
   ...
   if (!_sound_installed) {
      strcpy(allegro_error, get_config_text("Sound system not installed"));
      return -1;
   }
   ...
   if (digi_input_card == DIGI_AUTODETECT)
      digi_input_card = fix_digi_id(get_config_id("sound", "digi_input_card", DIGI_AUTODETECT));

   if (midi_input_card == MIDI_AUTODETECT)
      midi_input_card = fix_midi_id(get_config_id("sound", "midi_input_card", MIDI_AUTODETECT));

   allegro_error[0] = 0;

   /* search table for a good digital input driver */
   for (c=0; (_digi_driver_list[c].driver) && (_digi_driver_list[c].driver != &digi_none); c++) { 
      if ((_digi_driver_list[c].driver_id == digi_input_card) || 
	  (digi_input_card == DIGI_AUTODETECT)) {
	 digi_input_driver = _digi_driver_list[c].driver;
	 if (digi_input_driver->detect(TRUE)) {
	    digi_input_card = _digi_driver_list[c].driver_id;
	    break;
	 }
	 else {
	    ...
	    if (digi_input_card != DIGI_AUTODETECT) {
	       if (!allegro_error[0])
		  sprintf(allegro_error, get_config_text("%s does not support audio input"), _digi_driver_list[c].driver->name);
	       break;
	    }
	 }
      }
   ...
   for (c=0; (_midi_driver_list[c].driver) && (_midi_driver_list[c].driver != &midi_none); c++) { 
      if ((_midi_driver_list[c].driver_id == midi_input_card) || 
	  (midi_input_card == MIDI_AUTODETECT)) {
	 midi_input_driver = _midi_driver_list[c].driver;
	 if (midi_input_driver->detect(TRUE)) {
	    midi_input_card = _midi_driver_list[c].driver_id;
	    break;
	 }
	 else {
	    ...
	    if (midi_input_card != MIDI_AUTODETECT) {
	       if (!allegro_error[0])
		  sprintf(allegro_error, get_config_text("%s does not support MIDI input"), _midi_driver_list[c].driver->name);
	       break;
	    }
	 }
      }
