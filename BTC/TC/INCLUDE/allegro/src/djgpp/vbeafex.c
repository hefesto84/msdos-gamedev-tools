 ...
 * all this code just to use their drivers. It is also very irritating 
 * that they had to go and define their own register structure, which is 
 * totally incompatible with the standard DPMI one that everyone else uses!
 */
 ...
 */
static void call_real_mode(unsigned int seg, unsigned int off, SILLY_SCITECH_REGS *regs, SILLY_SCITECH_SREGS *sregs)
{
   __dpmi_regs r;
 ...
 */
static int my_int86(int intno, SILLY_SCITECH_REGS *in, SILLY_SCITECH_REGS *out)
{
   __dpmi_regs r;
 ...
 *  Generates a real mode interrupt.
 */
static int my_int86x(int intno, SILLY_SCITECH_REGS *in, SILLY_SCITECH_REGS *out, SILLY_SCITECH_SREGS *sregs)
{
   __dpmi_regs r;
 ...
 */
static void segread(SILLY_SCITECH_SREGS *sregs)
{
   asm (
      "  movw %%cs, %w0 ; "
      "  movw %%ds, %w1 ; "
 ...
 */
static int my_int386x(int intno, SILLY_SCITECH_REGS *in, SILLY_SCITECH_REGS *out, SILLY_SCITECH_SREGS *sregs)
{
   asm (
 ...
 *  Generates a protected mode interrupt.
 */
static int my_int386(int intno, SILLY_SCITECH_REGS *in, SILLY_SCITECH_REGS *out)
{
...
{
   _farpokeb(_dos_ds, 0x450, x);
   _farpokeb(_dos_ds, 0x451, y);
}
...
{
   _farpokeb(_dos_ds, 0x44A, width);
   _farpokeb(_dos_ds, 0x484, height-1);
}
