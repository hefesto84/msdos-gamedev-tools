...
static int fm_detect(int input)
{
   ...
   if (!fm_is_there()) {
      strcpy(allegro_error, get_config_text("OPL synth not found"));
      return FALSE;
   }
   ...
   if (midi_card == MIDI_OPL3) {
	 ...
	 strcpy(allegro_error, get_config_text("OPL3 synth not found"));
	 return FALSE;
      }
   }
   else if (midi_card == MIDI_2XOPL2) {
      if (opl_type != MIDI_2XOPL2) {
	 strcpy(allegro_error, get_config_text("Second OPL2 synth not found"));
	 return FALSE;
      }
   }
...
static int fm_init(int input, int voices)
{
   ...
   for (i=0; i<2; i++) {
      ...
      if ((s) && (s[0])) {
	 if (load_ibk(s, (i > 0)) != 0) {
	    sprintf(allegro_error, get_config_text("Error reading .IBK file '%s'"), s);
	    return -1;
	 }
      }
   }
