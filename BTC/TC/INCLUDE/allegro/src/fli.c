...
static void do_fli_256_color(unsigned char *p, int sz)
{
   ...
   while (packets-- > 0) {
	 ...
	 fli_pallete[offset].r = READ_BYTE_NC(p) / 4;
	 fli_pallete[offset].g = READ_BYTE_NC(p) / 4;
	 fli_pallete[offset].b = READ_BYTE_NC(p) / 4;
      }
   }
}
...
static void do_fli_color(unsigned char *p, int sz)
{
   ...
   while (packets-- > 0) {
	 ...
	 FLI_KLUDGE(p, sz, length * 3);
      ...
      for(; offset < end; offset++) {
	 fli_pallete[offset].r = READ_BYTE_NC(p);
	 fli_pallete[offset].g = READ_BYTE_NC(p);
	 fli_pallete[offset].b = READ_BYTE_NC(p);
      }
   }
}
...
static void do_fli_copy(unsigned char *p, int sz)
{
   if ((sz -= (fli_bitmap->w * fli_bitmap->h)) < 0)
      return;
#ifdef DJGPP
   READ_BLOCK_NC(p, fli_bitmap->dat, fli_bitmap->w * fli_bitmap->h);
#else
   {
      int y;
...
int open_fli(char *filename)
{
   ...
   fli_filename = malloc(strlen(filename)+1);
   if (!fli_filename)
      return FLI_ERROR;
   strcpy(fli_filename, filename);
